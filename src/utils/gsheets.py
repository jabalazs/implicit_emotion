...
class Worksheet(object):
    ...
    def get_first_empty_cell_coords(self):
        return (self.get_last_row_idx() + 1,
                self.get_first_col_idx())
    ...
    def insert_row_from_cell(self, row, row_idx, col_idx):
        grange = self.gsheet.range(row_idx, col_idx, row_idx,
                                   col_idx + len(row) - 1)
    ...
    def batch_insert_rows_from_cell(self, rows, row_idx, col_idx):
        ...
        grange = self.gsheet.range(row_idx, col_idx,
                                   row_idx + num_rows - 1,
                                   col_idx + len_rows - 1)
    ...
    def insert(self, datadict):
        ...
        if self.is_empty:
            self.batch_insert_rows_from_cell([ordered_dict.keys(),
                                              ordered_dict.values()],
                                             1,
                                             1)
            self.is_empty = False
            ...
            if set(header) == set(ordered_dict.keys()):
                self.insert_row_from_cell(ordered_dict.values(),
                                          ins_row_idx,
                                          ins_col_idx)

            else:
                print('Header is different from keys, beginning debugging session')
